...
class Logger:
    ...
    def __init__(self, name: str, *, use_date_prefix=True, print_logs=True, write_logs=True, log_dir=None, log_filename='%Y-%m-%d'):
        ...
        Args:
            ...
            log_filename (str, optional): Filename for log path, should comply strftime function requirements. Defaults to '%Y-%m-%d'.
        ...
        if write_logs == True and (log_filename == None or log_dir == None):
            raise ValueError("Wrong arguments, log_filename and log_dir cannot be None when write_logs is True")
    ...
    def __log_path(self):
        return os.path.join(self.log_dir, 'logs', datetime.today().strftime(self.log_filename) + '.txt')
    ...
    def log(self, message=None, *, write_to_console=None, console_add_prefix=True, write_to_log=None, log_add_prefix=True):
        """Logs desired message

        Args:
            message (str, optional): Message to be logged. Defaults to None.
        ...
        if write_to_console or (write_to_console == None and self.print_logs):
            print((self.__full_prefix() if console_add_prefix else '') + ('' if message == None else message))
        if write_to_log == True or (write_to_log == None and self.write_logs):
            ...
            with open(self.__log_path(), 'a', encoding='UTF8') as fdesc:
                fdesc.write((self.__full_prefix() if log_add_prefix else '') + ('' if message == None else message) + '\n')